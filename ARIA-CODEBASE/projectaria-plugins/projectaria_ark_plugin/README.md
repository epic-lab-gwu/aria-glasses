...
| custom-profile | Author a custom profile JSON for Aria Gen 2 — pull the right pre-defined base off the device, change parameters conservatively, validate, add it back. A profile works for both recording and streaming. Encodes the risks (thermal, MPS rejection) of custom profiles. |
...
| timecode-bridge | Time-align Aria Gen 2 recordings with external LTC-timecoded equipment (motion capture systems, professional cameras, audio recorders) via [Timecode Bridge](https://github.com/facebookresearch/projectaria_timecode_bridge) — an open-source LTC-to-SubGHz hardware companion that plugs into Aria's built-in Time Domain Mapping as an alternative broadcaster. |
